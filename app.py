...
@app.route('/data/clusters/rerun', methods=['POST'])
def api_clusters_rerun():
    from algorithms.kmeans_cluster import run_kmeans_custom
    data = request.get_json()
    k = int(data.get('k', 5))
    k = max(2, min(8, k))  # safety: بين 2 و 8
    clusters = run_kmeans_custom(df, k)
    return jsonify({'clusters': clusters})
